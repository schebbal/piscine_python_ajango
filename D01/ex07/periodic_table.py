...
def header(f):
    ...
    f.write(
        "    <style>table {	width: 100%;} td {border: 1px solid black; padding:10px} </style>\n  </head>\n")
...
def content(f, tab):
    ...
    for elem in tab:
        ...
        if current - previous > 1:
            s += "        <td colspan='" + str(current - previous - 1) + "'>\n        </td>\n"
        s += "        <td><h4>" + elem['name'] + "</h4>\n"
        s += "          <ul><li>" + elem['number'] + "</li><li>" + \
            elem['small'] + "</li><li>" + elem['molar'] + "</li></ul>\n        </td>\n"
        if current == 17:
            s += "      </tr>\n"
            current = 0
        previous = current
